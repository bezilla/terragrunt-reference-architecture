...
Datadog monitors as data: one `for_each` factory taking a map of monitor definitions (metric/event/query alerts with thresholds, priority, and paging targets). **Consolidates eleven near-identical monitor modules** from the source system into a single data-driven module. Notification targets are a variable with generic defaults — no team handle baked in. Provider (API/app keys) is configured by the caller.
